...
import { span } from "@oisasoje/gloo";
...
export async function startAuth(phone) {
    const user = await span("prisma.users.findUnique", () => prisma.users.findUnique({
        where: { phone },
    }));
...
export async function verifyAuth(id, pin) {
    ...
    if (!user)
        throw new Error("Something unexpected occurred. Try again later.");
    const targetHash = user.pin_hash || DUMMY_HASH;
    const valid = await verifyPin(targetHash, pin);
    if (!valid) {
        throw new Error("Invalid credentials");
    }
    const session = await span("prisma.session.create", () => prisma.session.create({
        data: {
...
export async function getSession(sessionId) {
    const session = await span("prisma.session.findUnique", () => prisma.session.findUnique({
        where: { id: sessionId },
        include: { user: true },
    }));
    if (!session)
    ...
    if (session.expires_at < new Date()) {
        await span("prisma.session.deleteMany", () => prisma.session.deleteMany({
            where: { id: session.id },
        }));
        return null;
    }
